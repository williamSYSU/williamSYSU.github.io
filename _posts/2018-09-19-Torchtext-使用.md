...
-   *use\_vocab*: 是否使用词典对象.
    如果是False，数据的类型必须已经是数值类型. 默认值: True.

    > 如果Label是整形，则不需要再转成index，则需要设置成*user\_vocab=False*。
...
-   *preprocessing*: 在分词之后和数值化之前使用的管道 默认值: None.
    ...
    > 一个自定义的预处理函数，函数的输入是分词之后的词列表，类型为*list*
    >
    > 如，自定义一个清洗符号的函数：
...
-   构建词典。在使用数据前，必须建立对应Field的词典。
    ...
    >
    > 默认的预训练数据在.vector\_cache/文件夹中，可以自己事先下载后对应数据，解压后放到该文件夹中，vectors参数中把文件后缀名去掉。
...
> *note*:
> 如果不设置Field的fix\_length，则数据长度默认取每个batch中的最长序列，其它不足的用填充符填充。
...
首先定义分词函数，定义Field类，其中指定需要如何预处理文本数据。
...
在两个Field中，加入了 *batch\_first=True*
这个参数，是为了在训练时更好地识别数据。

在LABEL中，定义了
*user\_vocab=False*，因为在使用中，我的Label数据是整形数据。